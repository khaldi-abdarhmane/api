...
def main():
    ...
    validation_data_path = delete_spaces(
        args.validation_data_path
    )  #  validation data path
    output_model_path = delete_spaces(
        args.model_artifact_path
    )  # model artifact folder path: where we stock the model
    output_history_path = delete_spaces(
        args.history_df_path
    )  # history folder path : where we stock this file
    run_info_path = delete_spaces(
        args.run_info_path
    )  #  the path where to save run_info.json :
    ...
    base_model_1 = VGG16(include_top=params.include_top, input_shape=params.input_shape)
    arch_model = Arch_model(
        base_model=base_model_1, class_number=generatorobjet.class_number
    )
    ...
    with mlflow.start_run() as run:
        ...
        history = model_.fit(
            generatorobjet.train_generator,
        ...
        logger.info(
            "> start upload training artifact to mlflow serve".format(run.info.run_id)
        )
        mlflow.log_params(params_dict)
        mlflow.log_artifact(history_path)
        mlflow.keras.log_model(model_, "keras")
        logger.info(
            "> finish upload training artifact to mlflow serve".format(run.info.run_id)
        )
